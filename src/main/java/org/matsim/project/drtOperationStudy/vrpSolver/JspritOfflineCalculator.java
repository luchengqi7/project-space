...
public class JspritOfflineCalculator {
    public record Options(boolean infiniteFleet, boolean printProgressStatistics, int maxIterations,
                          boolean multiThread) {
    ...
    public JspritOfflineCalculator(DrtConfigGroup drtCfg, FleetSpecification fleetSpecification, Network network,
    ...
    public PreplannedDrtOptimizer.PreplannedSchedules calculate() {
        ...
        // run jsprit
        var problem = vrpBuilder.setFleetSize(options.infiniteFleet ? VehicleRoutingProblem.FleetSize.INFINITE : VehicleRoutingProblem.FleetSize.FINITE).build();
