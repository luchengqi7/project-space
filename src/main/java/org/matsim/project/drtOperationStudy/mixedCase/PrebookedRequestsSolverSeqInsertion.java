...
public class PrebookedRequestsSolverSeqInsertion implements PrebookedRequestsSolver {
    private final Network network;
    private final TravelTime travelTime;
    private final double stopDuration;

    PrebookedRequestsSolverSeqInsertion(Network network, TravelTime travelTime, DrtConfigGroup drtConfigGroup) {
    ...
    public MixedCaseDrtOptimizer.FleetSchedules calculate(MixedCaseDrtOptimizer.FleetSchedules previousSchedules,
                                                          ...
                                                          List<MixedCaseDrtOptimizer.GeneralRequest> newRequests, double time) {
        ...
        // Vehicle locations
        for (MixedCaseDrtOptimizer.OnlineVehicleInfo onlineVehicleInfo : onlineVehicleInfoMap.values()) {
        ...
        for (List<TimetableEntry> timetable : previousSchedules.vehicleToTimetableMap().values()) {
            for (TimetableEntry timetableEntry : timetable) {
                if (timetableEntry.stopType == TimetableEntry.StopType.PICKUP) {
        ...
        // new requests
        for (MixedCaseDrtOptimizer.GeneralRequest request : newRequests) {
        ...
        for (MixedCaseDrtOptimizer.GeneralRequest request : newRequests) {
            ...
            for (Id<DvrpVehicle> vehicleId : previousSchedules.vehicleToTimetableMap().keySet()) {
                MixedCaseDrtOptimizer.OnlineVehicleInfo vehicleInfo = onlineVehicleInfoMap.get(vehicleId);
    ...
    // Private classes / records
    private static class LinkToLinkTravelTimeMatrix {
    ...
    private void updateFleetSchedule(MixedCaseDrtOptimizer.FleetSchedules previousSchedules,
                                     Map<Id<DvrpVehicle>, MixedCaseDrtOptimizer.OnlineVehicleInfo> onlineVehicleInfoMap,
        ...
        for (Id<DvrpVehicle> vehicleId : previousSchedules.vehicleToTimetableMap().keySet()) {
            ...
            if (!timetable.isEmpty()) {
                ...
                for (TimetableEntry timetableEntry : timetable) {
                    Id<Link> stopLinkId = timetableEntry.stopType == TimetableEntry.StopType.PICKUP ? timetableEntry.getRequest().fromLinkId() : timetableEntry.getRequest().toLinkId();
